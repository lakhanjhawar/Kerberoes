...
{
	union {
		keyreq requestsessionkey_1_arg;
	} argument;
	char *result;
	...
	case REQUESTSESSIONKEY:
		_xdr_argument = xdr_keyreq;
		_xdr_result = xdr_totalreply;
...
main()
{
	...
	if (t_getstate(0) != -1 || t_errno != TBADF) {
		...
		if (!svc_reg(transp, CS_PROG, AS_VERS, cs_prog_1, 0)) {
			_msgout("unable to register (CS_PROG, AS_VERS).");
			exit(1);
		}
		if (pmclose) {
			(void) signal(SIGALRM, (void(*)()) closedown);
...
#pragma weak closefrom
	...
	if (!svc_create(cs_prog_1, CS_PROG, AS_VERS, "netpath")) {
		_msgout("unable to create (CS_PROG, AS_VERS) for netpath.");
		exit(1);
	}
