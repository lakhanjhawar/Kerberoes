...
main()
{
	...
	if (t_getstate(0) != -1 || t_errno != TBADF) {
		...
		if (!svc_reg(transp, SP_PROG, AS_VERS, sp_prog_1, 0)) {
			_msgout("unable to register (SP_PROG, AS_VERS).");
			exit(1);
		}
		if (pmclose) {
			(void) signal(SIGALRM, (void(*)()) closedown);
...
#pragma weak closefrom
	...
	if (!svc_create(sp_prog_1, SP_PROG, AS_VERS, "netpath")) {
		_msgout("unable to create (SP_PROG, AS_VERS) for netpath.");
		exit(1);
	}
