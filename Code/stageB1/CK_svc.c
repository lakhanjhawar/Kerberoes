...
static encryptedtoken *
_requestsessionkey_1(argp, rqstp)
	tokenrqst  *argp;
	struct svc_req *rqstp;
{
	return (requestsessionkey_1(*argp, rqstp));
}

static void
cs_prog_1(rqstp, transp)
	struct svc_req *rqstp;
	register SVCXPRT *transp;
{
	union {
		tokenrqst requestsessionkey_1_arg;
	} argument;
	char *result;
	...
	case REQUESTSESSIONKEY:
		_xdr_argument = xdr_tokenrqst;
		_xdr_result = xdr_encryptedtoken;
		local = (char *(*)()) _requestsessionkey_1;
		break;
...
main()
{
	...
	if (t_getstate(0) != -1 || t_errno != TBADF) {
		...
		if (!svc_reg(transp, CS_PROG, CS_VERS, cs_prog_1, 0)) {
			_msgout("unable to register (CS_PROG, CS_VERS).");
			exit(1);
		}
		if (pmclose) {
			(void) signal(SIGALRM, (void(*)()) closedown);
...
#pragma weak closefrom
	...
	if (!svc_create(cs_prog_1, CS_PROG, CS_VERS, "netpath")) {
		_msgout("unable to create (CS_PROG, CS_VERS) for netpath.");
		exit(1);
	}
