...
static tokenrply2 *
_strfunc_1(argp, rqstp)
	clntreqtoken  *argp;
	struct svc_req *rqstp;
{
	return (strfunc_1(*argp, rqstp));
}
	...
	register SVCXPRT *transp;
{
	...
	case STRFUNC:
		_xdr_argument = xdr_clntreqtoken;
		_xdr_result = xdr_tokenrply2;
		local = (char *(*)()) _strfunc_1;
		break;
...
main()
{
	...
	if (t_getstate(0) != -1 || t_errno != TBADF) {
		...
		if (!svc_reg(transp, SP_PROG, SP_VERS, sp_prog_1, 0)) {
			_msgout("unable to register (SP_PROG, SP_VERS).");
			exit(1);
		}
		if (pmclose) {
			(void) signal(SIGALRM, (void(*)()) closedown);
...
#pragma weak closefrom
	...
	if (!svc_create(sp_prog_1, SP_PROG, SP_VERS, "netpath")) {
		_msgout("unable to create (SP_PROG, SP_VERS) for netpath.");
		exit(1);
	}
